...
class ALResourceManagerProxyPostHandlerRemote : public ALProxyRemote
{
  ...
  public:
    ...
    {
      return fParent->pCall("createResourcesList" , resourceGroupName, resourceName);
    }
    ...
    /// <param name="timeoutSeconds"> Timeout in seconds </param>
    /// <returns> brokerTaskID : The ID of the task assigned to it by the broker. </returns>
    int waitForResourcesTree(const std::vector<std::string>& ressourceName, const std::string& ownerName, const std::string& callbackName, const int& timeoutSeconds)
    {
      return fParent->pCall("waitForResourcesTree" , ressourceName, ownerName, callbackName, timeoutSeconds);
    }
