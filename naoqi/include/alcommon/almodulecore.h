...
/**
 * \def BIND_OBJ_METHOD(objptr, meth)
 * \brief Bind a method \a meth to an object \a objptr.
 */
...
/**
 * \def BIND_METHOD(x)
 * \brief Bind a method \a x to an the current object (\a this). It will be
 * added to the module API. The bound function can be then called locally or
 * remotely.
 */
...
namespace AL
{
   ...
   * \ingroup Module
   */
  class ALModuleCore: public ::boost::enable_shared_from_this<ALModuleCore>, public ::boost::noncopyable
  {
    friend class baseModule;
  private:
    /** Pointer to private ALModuleCore implementation */
    boost::shared_ptr<ALModuleCoreImpl> fImpl;
  ...
  protected:
     ...
     */
    void setReturn(const std::string &pName,
                   const std::string &pDesc);
  };  // !ALModuleCore
}
