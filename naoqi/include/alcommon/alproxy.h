...
namespace AL
{
  ...
  protected:
    void CALL_METHODS(const std::string &szMethodName,
                      const AL::ALValue &param,
                      AL::ALValue &result);
    void PCALL_METHODSNoID(const std::string &szMethodName,
                           const AL::ALValue &param,
                           AL::ALValue &result);
    void PCALL_METHODS(const std::string &szMethodName,
                       const AL::ALValue &param,
                       AL::ALValue &result);
    /** \endcond */
  ...
  private:
     ...
     */
    bool isScript(void);
    void init(boost::shared_ptr<ALBroker> pBroker,
              const std::string &pModuleName,
              int pProxyMask = 0,
              int timeout = 0);
    void init(const std::string& pModuleName,
              const std::string& pIpParam,
              int pPort,
              boost::shared_ptr<ALBroker> pBroker,
              int pProxyOption,
              int timeout);
    ...
    std::string xCleanEndpointAddress(const std::string &pEndpoint);
    AL::ALValue xWaitIfNecessary(AL::ALValue result);

    boost::shared_ptr<detail::ALTaskInfo> xCreateTaskInfo(const std::string pMethodName);
    void xAddTask(const std::string &name, detail::PCallBase *pcall);
    ALMethodInfo* xLookupMethod(const std::string &pName);
    ALMethodInfo* xLookupMethod(const std::string &pMethodName,
                                const AL::ALValue &pParams);
    ALMethodInfo* xLookupMethod(const std::string &pMethodName,
                                const std::vector<std::string> &pParams,
                                bool softCompare);
    const std::string& xGetModuleName();
    void xExecute(const std::string &pMethod,
                  const ALValue &pParams,
                  ALValue &pResult);

    int xPCallScript(const std::string &name,
                     const ALValue &param);

  private:
    /** \brief Private implementation of ALProxy */
    ALProxyImpl *_private;
